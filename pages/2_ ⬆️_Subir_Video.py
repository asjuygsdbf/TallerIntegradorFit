...
st.title('Entrenador Fitness: Taller integrador')
...
with st.form('Upload', clear_on_submit=True):
    up_file = st.file_uploader("Subir Video", ['mp4','mov', 'avi'])
    uploaded = st.form_submit_button("Subir")

stframe = st.empty()

ip_vid_str = '<p style="font-family:Helvetica; font-weight: bold; font-size: 16px;">Video Entrada</p>'
warning_str = '<p style="font-family:Helvetica; font-weight: bold; color: Red; font-size: 17px;">Suba un video primero</p>'
...
if os.path.exists(output_video_file):
    with open(output_video_file, 'rb') as op_vid:
        download = download_button.download_button('Descargar Video', data = op_vid, file_name='output_recorded.mp4')
