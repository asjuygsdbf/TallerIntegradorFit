...
class SquatProcessor:
    def __init__(self, thresholds, flip_frame = False):
        ...
        self.state_tracker = {
            ...
            'iniciar_tiempo_inactivo': time.perf_counter(),
            'iniciar_tiempo_inactivo_front': time.perf_counter(),
            'TIEMPO_INACTIVO': 0.0,
            ...
            'CONTADOR_SENTADILLAS': 0,
            'SENTADILLA_INCORRECTA':0
        ...
        self.FEEDBACK_ID_MAP = {
                                ...
                                2: ('RODILLA MÁS ATRAS DE LOS PULGARES', 170, (255, 80, 80)),
                                3: ('SENTADILLA MUY BAJA', 125, (255, 80, 80))
                               }
    ...
    def _show_feedback(self, frame, c_frame, dict_maps, BAJA_CADERAS_disp):


        if BAJA_CADERAS_disp:
            draw_text(
                    frame, 
                    'BAJAR LAS CADERAS', 
                    pos=(30, 80),
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                self.state_tracker['INACTIVE_TIME_FRONT'] += end_time - self.state_tracker['iniciar_tiempo_inactivo_front']
                self.state_tracker['iniciar_tiempo_inactivo_front'] = end_time

                if self.state_tracker['INACTIVE_TIME_FRONT'] >= self.thresholds['INACTIVE_THRESH']:
                    self.state_tracker['CONTADOR_SENTADILLAS'] = 0
                    self.state_tracker['SENTADILLA_INCORRECTA'] = 0
                ...
                if display_inactivity:
                    # cv2.putText(frame, 'Resetting CONTADOR_SENTADILLAS due to inactivity!!!', (10, frame_height - 90), 
                    #             self.font, 0.5, self.COLORS['blue'], 2, lineType=self.linetype)
                    play_sound = 'reset_counters'
                    self.state_tracker['INACTIVE_TIME_FRONT'] = 0.0
                    self.state_tracker['iniciar_tiempo_inactivo_front'] = time.perf_counter()

                draw_text(
                    frame, 
                    "CORRECTA: " + str(self.state_tracker['CONTADOR_SENTADILLAS']), 
                ...
                draw_text(
                    frame, 
                    "INCORRECTA: " + str(self.state_tracker['SENTADILLA_INCORRECTA']), 
                ...
                draw_text(
                    frame, 
                    'ANGULO DE COMPENSACION: '+str(offset_angle), 
                ...
                self.state_tracker['iniciar_tiempo_inactivo'] = time.perf_counter()
                self.state_tracker['TIEMPO_INACTIVO'] = 0.0
            ...
            # Camera is aligned properly.
                ...
                if current_state == 's1':
                    ...
                    if len(self.state_tracker['state_seq']) == 3 and not self.state_tracker['POSTURA_INCORRECTA']:
                        self.state_tracker['CONTADOR_SENTADILLAS']+=1
                        play_sound = str(self.state_tracker['CONTADOR_SENTADILLAS'])
                        
                    elif 's2' in self.state_tracker['state_seq'] and len(self.state_tracker['state_seq'])==1:
                        self.state_tracker['SENTADILLA_INCORRECTA']+=1
                        play_sound = 'INCORRECTA'

                    elif self.state_tracker['POSTURA_INCORRECTA']:
                        self.state_tracker['SENTADILLA_INCORRECTA']+=1
                        play_sound = 'INCORRECTA'
                        
                    
                    self.state_tracker['state_seq'] = []
                    self.state_tracker['POSTURA_INCORRECTA'] = False
                ...
                if self.state_tracker['curr_state'] == self.state_tracker['prev_state']:
                    ...
                    end_time = time.perf_counter()
                    self.state_tracker['TIEMPO_INACTIVO'] += end_time - self.state_tracker['iniciar_tiempo_inactivo']
                    self.state_tracker['iniciar_tiempo_inactivo'] = end_time

                    if self.state_tracker['TIEMPO_INACTIVO'] >= self.thresholds['INACTIVE_THRESH']:
                        self.state_tracker['CONTADOR_SENTADILLAS'] = 0
                        self.state_tracker['SENTADILLA_INCORRECTA'] = 0
                        display_inactivity = True
                ...
                if 's3' in self.state_tracker['state_seq'] or current_state == 's1':
                    self.state_tracker['BAJA_CADERAS'] = False

                self.state_tracker['COUNT_FRAMES'][self.state_tracker['DISPLAY_TEXT']]+=1

                frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, self.state_tracker['BAJA_CADERAS'])
            ...
            if self.state_tracker['TIEMPO_INACTIVO'] >= self.thresholds['INACTIVE_THRESH']:
                self.state_tracker['CONTADOR_SENTADILLAS'] = 0
                self.state_tracker['SENTADILLA_INCORRECTA'] = 0
                # cv2.putText(frame, 'Resetting CONTADOR_SENTADILLAS due to inactivity!!!', (10, frame_height - 25), self.font, 0.7, self.COLORS['blue'], 2)
                display_inactivity = True
